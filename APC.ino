...
const char APC_Version[6] = "01.05";                  // Current APC version - includes the other INO files also
...
byte LED_Counter = 0;                                 // issues the sync
byte LED_BufferRead = 0;                              // read pointer for ringbuffer SpcBuffer
byte LengthOfSyncCycle = 3;                           // stores the length of the LED sync cycle in ms
volatile byte LED_BufferWrite = 0;                    // write pointer for ringbuffer SpcBuffer
bool LED_Select_Polarity = 0;                         // LED_Exp board uses rising and falling edge of Sel 5
...
bool USBlog = 0;                                      // write log data to USB if set
...
const char TxTDebugSelect[3][17] = {{"        OFF     "},{"       USB ON   "},{"       USB LOG  "}};
...
void setup() {
  ...
  SolChange = true;                                   // enable solenoid handling
...
void Init_System2(byte State) {                       // state = 0 will restore the settings if no card is found
  ...
  if (APC_settings[DebugMode] == 2) {                 // USB LOG selected
    USBlog = true;                                    // enable USB log output
    ...
    USBlog = false;}                                  // disable USB log output
  ...
  if (LED_Counter) {
    ...
    if (LED_Send) {                                   // bytes  to be sent?
      ...
      if (LED_Select_Polarity) {                      // sel5 has to toggle for each cycle
        REG_PIOC_CODR = 2097152;
        LED_Select_Polarity = false;}
      else {
        REG_PIOC_SODR = 2097152;
        LED_Select_Polarity = true;}
      REG_PIOC_CODR = Sel14;}}                        // disable the HW_ext_latch
...
void ReadMusic() {                                    // read music data from SD
  if (MusicFile.available() > 255) {                  // enough data remaining in file to fill one block?
    ...
    for (byte i=0; i<128-Avail/2; i++) {                   // fill the rest with silence
...
void WritePlayerDisplay(char* DisplayText, byte Player) { // write ASCII text to player displays - credit is Player 0
  ...
  case 2:                                             // Sys11 F-14
    if (Player) {                                     // player display?
      if (Player < 3) {                               // upper row?
        Player--;
        for (byte i=0;i<7;i++) {                      // for all digits
  ...
  case 6:                                             // DE 2x16
    if (Player == 1) {
      for (byte i=0;i<16;i++) {                       // for all digits
        if (*(DisplayText+i) & 128) {                 // comma set?
  ...
  case 7:                                             // Sys6 display
    if (Player) {                                     // player display?
      if (Player < 3) {                               // upper row?
        Player--;
        for (byte i=0;i<6;i++) {                      // for all digits
...
void AddScrollUpper(byte Step) {                      // call with Step = 0 and the text being in DisplayUpper2 / stop with Step = 100
  ...
  if (Step == 100) {                                  // stop command
    ...
    if (Step > 8) {                                   // scroll into the left display?
      for (byte i=0; i<2*(Step-9);i++) {                   // for all characters in the left display that have to be scrolled
      ...
      DisplayUpper[15] = DisplayUpper[19];
      for (byte i=0; i<12; i++) {                     // scroll the right display by 6 characters
        DisplayUpper[18+i] = DisplayUpper[20+i];}}
    else {                                            // scroll only the right display
      for (byte i=0; i<2*(Step-1); i++) {
...
void ScrollLower(byte Step) {                         // call with Step = 0 and the text being in DisplayLower2 / stop with Step = 100
  ...
  if (Step == 100) {                                  // stop command
    ...
    if (Step < 8) {                                   // do display 3 first
      for (byte i=2; i<14; i++) {                     // scroll display 3
      ...
      for (byte i=18; i<30; i++) {                    // scroll display 4
        DisplayLower[i] = DisplayLower[i+2];}
...
void ReleaseAllSolenoids() {
  for (byte i=0; i< 4; i++) {                         // clear all solenoid buffers
    SolBuffer[i] = 0;}
...
byte HandleHighScores(unsigned int Score) {
  ...
  while (HallOfFame.Scores[Position] > Score) {       // check to which position of the highscore list it belongs
    Position++;}
  for (byte i=3; i>Position; i--) {                   // move all lower highscores down
    HallOfFame.Scores[i] = HallOfFame.Scores[i-1];}
  for (byte i=9; i>Position*3; i--) {
